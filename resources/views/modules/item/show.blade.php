  ...
  <div class="col-md-12 no-padding-left">
    <div class="col-md-8 no-padding-left">
      ...
      <div class="box box-success">
        <div class="box-body">
          <div>
            <img src="{{ $module->cover->path_string }}" class="item-image" alt="{{ $module->name }}" width="800"
                 height="140"/>
          </div>
          <div class="nav-tabs-custom">
            <ul class="nav nav-tabs">
              ...
              <li class=""><a href="#changelog" data-toggle="tab"
                              aria-expanded="false">{{ trans('modules.changelog') }}</a></li>
            </ul>
    ...
    <div class="col-md-4 no-padding-right">
      ...
      <div class="box box-success">
        ...
        <div class="box-footer">
          @if ($installed)
            <a href="{{ url('modules/item/' . $module->slug . '/uninstall') }}"
               class="btn btn-block btn-danger">{{ trans('modules.button.uninstall') }}</a>
            @if ($enable)
              <a href="{{ url('modules/item/' . $module->slug . '/disabled') }}"
                 class="btn btn-block btn-warning">{{ trans('modules.button.disable') }}</a>
            @else
              <a href="{{ url('modules/item/' . $module->slug . '/enabled') }}"
                 class="btn btn-block btn-success">{{ trans('modules.button.enable') }}</a>
            @endif
          @else
            <a href="{{ $module->action_url }}"
               class="btn btn-success btn-block" <?php echo ($module->install) ? 'id="install-module"' : 'target="_blank"'; ?>>
              @if ($module->install)
        ...
        <div class="box-body">
          <table class="table table-striped">
            ...
            <tr>
              <th>Vendor</th>
              <td><a
                  href="{{ url('modules/vendor/' . $module->vendor->id) }}">{{ $module->vendor->first_name . ' ' . $module->vendor->last_name }}</a>
              </td>
            </tr>
            <tr>
              <th>Version</th>
            ...
            <tr>
              <th>Category</th>
              <td><a href="{{ url('modules/category/' . $module->category->slug) }}">{{ $module->category->name }}</a>
              </td>
            </tr>
            </tbody>
          </table>
